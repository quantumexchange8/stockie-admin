...
class OrderController extends Controller
{
    ...
    {
        $reservedTableIds = Reservation::whereNotIn('status', ['Cancelled', 'Completed', 'No show', 'Checked in']) // Adjust status if needed
    ...
    public function kickDrawer()
    {
        ...
        $addText("THIS IS JUST A KICK DRAWER TEST!");
        $addText("by $printer->name");
        $addText(""); // Empty line
        $addText("Test generated by STOCKIE");
        
        // Add some empty lines and cut
        $addText("\n\n\n\n\n");
        $addCommand("\x1D\x56\x01"); // Partial cut
        $addCommand("\x1B\x70\x00\x78\x1E"); // Kick drawer 
        ...
        try {
            ...
            return response()->json([
                ...
                'data' => base64_encode($buffer), // Encode binary as base64
                'printer' => $printer,
                'message' => 'Kick drawer job sent'
            ]);
            
        } catch (\Exception $e) {
            \Log::error('Kick drawer receipt error: ' . $e->getMessage());
            return response()->json([
                'error' => 'Internal server error',
                'message' => $e->getMessage()
            ], 500);
