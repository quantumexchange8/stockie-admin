...
<template>
    <div class="flex flex-col gap-4 h-full">
        <div class="relative flex gap-5">
            <span class="text-md font-medium text-primary-900 whitespace-nowrap w-full">{{ $t('public.waiter.entitled_incentive') }}</span>
        ...
        <div class="flex flex-col px-0.5 items-start gap-4 self-stretch" v-if="configIncentive.length > 0" v-for="data in configIncentive" :key="data.incentive_id">
            ...
            class="w-full flex p-3 flex-col items-start gap-0.5 self-strech rounded-[5px] bg-primary-25">
                ...
                <span class="text-grey-900 text-sm font-normal">{{ $t('public.waiter.total_monthly_sales') }}</span>
        ...
        <div class="p-2 flex flex-col items-center text-center h-full justify-between self-stretch" v-else>
            ...
            <div class="flex flex-col gap-4 items-center text-center">
                <span class="text-sm font-medium text-primary-900 text-balance">{{ $t('public.empty.no_employee_incentive_comm') }}</span>
                <Button
                    :type="'button'"
