...
const csvExport = () => {
    ...
    const formattedRows = [
        ...
        { Product: wTrans('public.product').value, 'Sold': wTrans('public.sold').value, 'Gross (RM)': `${wTrans('public.report.gross').value} (RM)`, 'Disc. (RM)': `${wTrans('public.report.disc').value} (RM)`, 'Tax (RM)': `${wTrans('public.report.tax').value} (RM)`, 'Refund (RM)': `${wTransChoice('public.refund', 1).value} (RM)`, 'Net (RM)': `${wTrans('public.report.net').value} (RM)` },
        ...props.rows.map(row => ({
