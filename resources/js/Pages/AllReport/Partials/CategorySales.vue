...
const csvExport = () => {
    ...
    const formattedRows = [
        ...
        { Category: 'Category', 'Sold': 'Sold', 'Gross (RM)': 'Gross (RM)', 'Disc. (RM)': 'Disc. (RM)', 'Tax (RM)': 'Tax (RM)', 'Refund (RM)': 'Refund (RM)', 'Net (RM)': 'Net (RM)' },
        ...props.rows.map(row => ({
            'Category': row.name,
    ...
    exportToCSV(formattedRows, 'Category Sales Report');
}
