<?php
return [
    // ========================================================================
    //                      GLOBAL SHARED TERMS
    // ========================================================================

    "custom" => "Custom text here",
    "dashboard_header" => 'Dashboard',
    "order_management_header" => "Order Management",
    "shift_management_header" => "Shift Management",
    "shift_control_header" => "Shift Control",
    "shift_record_header" => "Shift Record",
    "menu_management_header" => "Menu Management",
    "all_report_header" => "All Report",
    "inventory_header" => "Inventory",
    "waiter_header" => "Waiter",
    "customer_header" => "Customer",
    "table_room_header" => "Table & Room",
    "reservation_header" => "Reservation",
    "transaction_listing_header" => "Transaction Listing",
    "einvoice_submission_header" => "e-Invoice Submission",
    "loyalty_programme_header" => "Loyalty Programme",
    "admin_user_header" => "Admin User",
    "sales_analysis_header" => "Sales Analysis",
    "activity_logs_header" => "Activity Logs",
    "configuration_header" => "Configuration",
    "general_header" => "General",
    "operation_header" => "Operation",
    "sales_management_header" => "Sales Management",
    "others_header" => "Others",
    "validation_error" => "Whoops! Something went wrong.",
    "all" => "All",
    "search" => "Search",
    "select" => "Select",
    "filter" => "Filter",
    "filter_by" => "Filter by",
    "date" => "Date",
    "item" => "Item",
    "almost_no_stock" => "is almost running out of stock!",
    "is_no_stock" => "is out of stock.",
    "product_affected" => "Product affected:",
    "view_stock" => "View Stock",
    "checked_in" => "Checked in",
    "checked_out" => "Checked out",
    "no_record_yet" => "No Record Yet",
    "checked_in_at" => "Checked-in at",
    "checked_out_at" => "Checked-out at",
    "table_room_activity" => "Table / Room Activity",
    "placed_order_for" => "placed an order for",
    "customer_checked_in_by" => "New customer check-in by",
    "assigned" => "Assigned",
    "to_serve" => "to serve",
    "merged_table" => "{0} merged table|{1} Merged table",
    "customer_check_in" => "Customer check-in",
    "place_order" => "Place order",
    "waiter_assignment" => "Waiter Assignment",
    "merge_table" => "Merge table",
    "transfer_table" => "Transfer table",
    "status" => "Status",
    "na" => "N/A",
    "sales" => "Sales",
    "left" => "left",
    "left_header" => "Left",
    "category" => "Category",
    "day" => "{0} day|{1} days",
    "week" => "Week",
    "month_header" => "Month",
    "month" => "{0} month|{1} months",
    "year" => "Year",
    "hour" => "hour",
    "product_name" => "Product Name",
    "assign_seat" => "Assign Seat",
    "date_time" => "Date & Time",
    "order_no" => "Order No.",
    "no_of_pax" => "No. of pax",
    "total" => "Total",
    "print_receipt" => "Print receipt",
    "showing" => "Showing",
    "go_to_page" => "Go to page",
    "customer_detail" => "Customer Detail",
    "order_header" => "Order",
    "point" => "{0} Point|| {1} Points",
    "point_lowercase" => "{0} point|| {1} points",
    "pts" => "pts",
    "checked_in_customer" => "Checked-in customer",
    "cancel_order" => "Cancel Order",
    "create_new_customer" => "Create New Customer",
    "guest" => "Guest",
    "current_points" => "Current Points",
    "current_tier" => "Current Tier",
    "keep_item_header" => "Keep Item",
    "view_history" => "View History",
    "kept_from" => "Kept from",
    "expire_on" => "Expire on",
    "kept_by" => "Kept by",
    "remark" => "Remark",
    "edit_kept_detail" => "Edit kept detail",
    "mark_expired" => "Mark as expired",
    "mark_expired_message" => "This kept item will be marked as expired and customer can no longer redeem back this item. Are you sure you want to proceed?",
    "delete_kept_item" => "Delete kept item",
    "confirmation" => "Confirmation",
    "edit_kept_item_detail" => "Edit Kept Item Detail",
    "from_order" => "From order",
    "kept" => "Kept",
    "expiry_date" => "Expiry date",
    "enter_here" => "Enter here",
    "history" => "History",
    "served" => "Served",
    "returned" => "Returned",
    "served_returned" => "Served/Returned",
    "expired" => "Expired",
    "edited" => "Edited",
    "deleted" => "Deleted",
    "keep" => "Keep",
    "keep_item" => "Keep Item",
    "kept_item_header" => "Kept Item",
    "kept_item" => "Kept item",
    "keep_handled_by" => "By",
    "detail" => "Detail",
    "served_by" => "Served by",
    "current_status" => "Current status",
    "quantity_cm" => "Quantity/cm",
    "at" => "at",
    "tier" => "Tier",
    "redeem_product" => "Redeem Product",
    "earned" => "Earned",
    "used" => "Used",
    "adjusted" => "Adjusted",
    "point_adjustment" => "Point Adjustment",
    "adjusted_by" => "Adjusted by",
    "point_expiration" => "Point Expiration",
    "entry_reward" => "Entry Reward",
    "entry_rewards" => "Entry Rewards",
    "tier_rewards" => "Tier Rewards",
    "active" => "Active",
    "inactive" => "Inactive",
    "redeemed" => "Redeemed",
    "entry_reward_for" => "Entry Reward for :rank_name",
    "min_spend" => "Min. spend",
    "no_min_spend" => "No min. spend",
    "discount" => "Discount",
    "bonus_point" => "Bonus Point",
    "redeemed_on" => "Redeemed on",
    "point_expiring_soon" => "points expiring soon",
    "expiring_soon" => "Expiring Soon",
    "expired_soon_message" => "This kept item will be expired on :expired_on.",
    "pending_action" => "Pending Action",
    "expired_keep_message" => "This item is expired. Please choose to expire it or extend the expiration date.",
    "set_header" => "Set",
    "set" => "set",
    "in_qty" => "in quantity",
    "in_cm" => "in cm",
    "months_from_today" => "{1} :count month starting from today|[2,*] :count months starting from today",
    "months_from_entry" => "{1} :count month starting from entry date|[2,*] :count months starting from entry date",
    "customise_range" => "Customise range...",
    "unsaved_changes" => "You've unsaved changes",
    "unsaved_changes_message" => "Are you sure you want to leave this page? The changes you made will not be saved.",
    "proceed_change_customer" => "Proceed with changing customer?",
    "proceed_change_customer_message" => "Switching the checked-in customer will remove the tier reward you've applied. Are you sure you want to proceed?",
    "price" => "Price",
    "qty" => "Qty",
    "cm" => "CM",
    "customer_name" => "Customer Name",
    "voucher_discount" => "Voucher Discount",
    "rounding" => "Rounding",
    "sub_total" => "Sub-total",
    "bill_discount" => "Bill Discount",
    "cash" => "Cash",
    "card" => "Card",
    "e_wallet" => "E-Wallet",
    "add_discount" => "Add Discount",
    "stackable" => "Stackable",
    "non_stackable" => "Not Stackable",
    "min_item_purchase" => "Min. :count item purchased",
    "voucher" => "Voucher",
    "redeemed_product" => "Redeemed Product",
    "reason" => "Reason",
    "enter_reason" => "Enter the reason",
    "sales_detail" => "Sales detail",
    "service_tax" => "Service tax",
    "refund" => "{0} Refund|{1} Refunds",
    "void" => "{0} Void|{1} Voids",
    "report_header" => "Report",
    "payment_method" => "Payment Method",
    "current_stock" => "Current Stock",
    "sold" => "Sold",
    "method" => "Method",
    "balance" => "Balance",
    "product" => "Product",
    "incentive" => "Incentive",
    "commission" => "Commission",
    "points_earned" => "Points Earned",
    "unit" => "Unit",
    "date_range" => "Date Range",
    "table_no" => "Table no.",
    "in_stock" => "In stock",
    "low_in_stock" => "Low in stock",
    "out_of_stock" => "{0} Out of stock||{1} Out of Stock",
    "stock_level" => "Stock Level",
    "total_stock" => "Total Stock",
    "select_image" => "Select an image",
    "drag_drop" => "Drag & drop",
    "or" => "or",
    "select_item" => "Select an item",
    "set_item_qty" => "Quantity of item in this set",
    "another_item" => "Another Item",
    "set_name" => "Set Name",
    "suggested_image_size" => "Suggested image size",
    "pixel" => "pixel",
    "redemption_allowed" => "Redemption is allowed",
    "redemption_not_allowed" => "Redemption is not allowed",
    "time" => "Time",
    "amount" => "Amount",
    "quantity" => "Quantity",
    "redeem_with" => "Redeem with",
    "redeemed_with" => "Redeemed with",
    "redeemed_by" => "Redeemed by",
    "stock" => "Stock",
    "select_category" => "Select Category",
    "keep_allowed" => "Keep is allowed",
    "keep_not_allowed" => "Keep is not allowed",
    "incorrect_info" => "Incorrect information",
    "duplicate_entry" => "Duplicate entry",
    "customer_asked_delete" => "Customer asked to delete",
    "others_specify_remark" => "Others (specify under Remarks)",
    "amount_discount" => "Discount (Amount)",
    "percent_discount" => "Discount (Percentage)",
    "free_item" => "Free Item",
    "this_month" => "This month",
    "this_year" => "This year",
    "total_sales" => "Total Sales",
    "pending" => "Pending",
    "delayed" => "Delayed",
    "completed" => "Completed",
    "no_show" => "No show",
    "cancelled" => "Cancelled",
    "paid" => "Paid",
    "go_view" => "Go view",
    "unit_type" => "Unit Type",
    "in" => "In",
    "out" => "Out",
    "action_header" => "Action",
    "extend_to" => "Extend to",
    "customer_checked_in" => "Customer checked-in",
    "minute" => "{0} minute|{1} minutes",
    "unknown" => "Unkown",
    "transaction_header" => "Transaction",
    "sales_transaction" => "Sales Transaction",
    "refund_transaction" => "Refund Transaction",
    "transaction_no" => "Transaction No.",
    "successful" => "Successful",
    "voided" => "Voided",
    "date_period" => "Date Period",
    "document_type" => "Document Type",
    "special_discount" => "Special Discount",
    "type" => "Type",
    "submitted" => "Submitted",
    "valid" => "Valid",
    "invalid" => "Invalid",
    "rejected" => "Rejected",
    "enabled" => "Enabled",
    "disabled" => "Disabled",
    "equal_to" => "equal to",

    // Input fields label name
    "field" => [
        "id" => "ID",
        "password" => "Password",
        "display_name" => "Display Name",
        "table_merged" => "Table merged",
        "assign_waiter" => "Assign waiter",
        "name" => "Name",
        "phone_no" => "Phone No.",
        "contact_no" => "Contact No.",
        "email" => "Email",
        "deletion_reason" => "Select Deletion Reason",
        "keep_for" => "Keep for",
        "enter_amount" => "Enter Amount",
        "full_name" => "Full Name",
        "staff_id" => "Staff ID",
        "salary_monthly" => "Salary per month (basic)",
        "salary_hourly" => "Salary per hour",
        "employment_type" => "Employment type",
        "waiter_position" => "Waiter Position",
        "clock_in_passcode" => "Clock in passcode",
        "passcode" => "Passcode",
        "delete_customer_reason" => "Reason for customer deletion",
        "group_name" => "Group name",
        "show_low_stock" => "Show low stock at",
        "select_date_time" => "Select date and time",
        "select_table_room" => "Select table/room",
        "select_enter_name" => "Select or enter guest name",
        "table_lock_before" => "Table lock before",
        "grace_period" => "Grace period",
        "reservation_date_time" => "Reservation date & time",
        "guest_name" => "Guest name",
        "reserved_by" => "Reserved by",
        "cancel_rsvp_reason" => "Select Cancellation Reason",
        "admin_id" => "Admin ID",
        "title" => "Title",
        "select_icon" => "Select an icon",
        "amount_spend_to_achieve" => "Amount spend to achieve this tier",
        "tier_entry_rewards" => "This tier is entitled to entry reward(s)",
        "reward" => "Reward",
        "reward_type" => "Reward Type",
        "select_reward_type" => "Select Reward Type",
        "reward_redeem_once" => "The reward can only be redeemed once.",
        "discount_amount" => "Discount Amount",
        "discount_percentage" => "Discount Percentage",
        "minimum_purchase_amount" => "Minimum purchase amount",
        "with_minimum_purchase" => "With minimum purchase",
        "bonus_point_get" => "Bonus point get",
        "select_product" => "Select a product",
        "unlock_table_after" => "Unlock Table After",
        "start_at" => "Start At",
        "promotion_active_period" => "Promotion Active Period",
        "description" => "Description",
    ],

    // Toast messages
    "toast" => [
        "login" => "You've logged in to STOXPOS.",
        "logout" => "You've logged out your STOXPOS account.",
        "changes_saved" => "Changes saved.",
        "update_profile_image" => "You've successfully changed your profile picture.",
        "selected_order_deleted" => "The selected order has been deleted.",
        "table_available_now" => "Selected table is now available for the next customer.",
        "order_cancelled" => "Selected order has been cancelled successfully.",
        "customer_added" => "Customer added successfully.",
        "customer_edit_success" => "Customer edited successfully.",
        "edit_success" => "Successfully edited.",
        "kept_item_expiry_extended_summary" => "Expiration date extended successfully",
        "kept_item_expiry_extended_detail" => "You’ve extended the expiration date for selected kept item.",
        "kept_item_expired_summary" => "Kept item successfully expired",
        "kept_item_expired_detail" => "Selected item has been expired and returned to inventory.",
        "kept_item_deleted_summary" => "Kept item successfully deleted",
        "kept_item_deleted_detail" => "Selected item has been deleted from this customer’s account.",
        "product_redeem_sucess_summary" => "Product redeemed successfully.",
        "product_redeem_sucess_detail" => "pts has been deducted from customer's point wallet",
        "order_item_placed_message" => "Product has been added to :table_no order.",
        "table_locked_summary" => "Table is currently locked",
        "table_locked_detail" => "This table is being viewed by another user. It will be unlocked after inactivity or by the user.",
        "item_kept_success_summary" => "Item kept successfully.",
        "item_kept_success_detail" => "You can always refer back the keep item in 'Customer detail'.",
        "merged_table_success" => "Selected table has been successfully merged with ':table_no'.",
        "unmerged_table_success" => "You’ve successfully unmerged the selected table(s).",
        "reassign_order_success" => "You’ve successfully re-assigned the order(s) to other table.",
        "transfer_all_success" => "You’ve successfully transfer the order to :table_no.",
        "transfer_items_success" => "You've successfully transfer the selected item from :from_table to :to_table.",
        "merged_bill_success" => "Selected bills have been merged into a single bill.",
        "payment_completed" => "Payment Completed",
        "payment_unsuccessful" => "Payment Unsuccessful",
        "total_amount_exceeded" => "Entered amount exceeded grand total amount.",
        "conflict_methods_summary" => "Conflicting payment method requirements",
        "conflict_methods_detail" => "The applied discounts' payment method requirements are conflicting with one another. Please resolve the conflict by choosing either one.",
        "required_methods_summary" => "Payment method requirements not met.",
        "required_methods_detail" => "Please use one of the required payment methods: :methods",
        "date_range_check" => "This discount cannot be applied as currently, it is not within the applicable period.",
        "day_of_week_check" => "{0} This discount cannot be applied as it is only available during weekdays.|{1} This discount cannot be applied as it is only available during weekends.",
        "time_window_check" => "This discount cannot be applied as currently, it is not within the applicable time period.",
        "criteria_check" => "{0} To be eligible for this discount, a min. spend of RM :discount_req is required.|{1} To be eligible for this discount, a min. quantity of :discount_req is required.",
        "tier_check" => "This discount cannot be applied as the currently checked-in customer has not yet reached the required tier.",
        "total_remaining_usage_limit_check" => "This discount's maximum number of usage has been exhausted thus, is currently unable to be applied.",
        "customer_usage_limit_check" => "This discount's maximum number of usage for this customer has been exhausted thus, is currently unable to be applied.",
        "non_applicable_check" => "':discount_name' cannot be applied.",
        "conflict_keep_detail" => "Discount ':discount_name' cannot be applied since it is not stackable with other discount.",
        "conflict_max_summary" => ":old_discount is replaced with ':new_discount'",
        "conflict_max_detail" => "As one of the discounts is not stackable, we've applied the discount with the highest value to maximise customer savings.",
        "shift_opened_success" => "Shift successfully opened. You're all set to start the day!",
        "double_shift_error_summary" => "You can't open more than one shift at the same time.",
        "double_shift_error_detail" => "To proceed, please close the current shift first.",
        "shift_closed_success" => "Shift successfully closed. Great job today!",
        "shift_pay_in_success" => "Pay in history for this shift has been updated.",
        "shift_pay_out_success" => "Pay out history for this shift has been updated.",
        "report_export_error" => "CSV export is not available on the currently viewed report.",
        "add_table_success" => "New table has been successfully added.",
        "table_deleted_success" => "Selected table has been deleted successfully.",
        "zone_added_success" => "New Zone has been added successfully.",
        "zone_edited_success" => "Zone name has been edited successfully.",
        "zone_deleted_success" => "Selected zone has been deleted successfully.",
        "zone_deleted_warning" => "Selected zone still has some table that are checked in.",
        "delete_table_warning" => "Unable to delete table.",
        "delete_table_warning_message" => "Table :table_no is currently checked in. To delete the table, it must be freed up first.",
        "edit_category_success" => "Category has been edited successfully.",
        "add_category_success" => "Category has been added successfully.",
        "delete_product_category_summary" => "Selected product category has been deleted.",
        "delete_product_category_detail" => "Products in this category have been moved to different categories.",
        "add_product_success" => "Product has been successfully added to your menu.",
        "delete_product_error" => "Selected product unable to be deleted.",
        "delete_product_success" => "Selected product has been successfully deleted.",
        "delete_product_item_success" => "Selected product item has been successfully deleted.",
        "add_waiter_success" => "New waiter has been successfully added.",
        "delete_waiter_success" => "Selected waiter has been successfully deleted",
        "keep_items_imported_success" => "Keep item list has been imported successfully.",
        "adjust_customer_point_success" => "Customer points adjusted successfully.",
        "delete_inventory_item_success" => "Inventory item has been successfully deleted",
        "add_group_summary" => "Group added successfully.",
        "add_group_detail" => "You can always add new stock to this group.",
        "add_product_to_menu_success" => "Products have been successfully added to your menu.",
        "replenish_stock_summary" => "Stock replenished successfully.",
        "replenish_stock_detail" => "You've replenished stock for :item_name",
        "reactivate_kept_item_summary" => "Kept item has been reactivated successfully",
        "reactivate_kept_item_detail" => "You’ve extended the expiration date for selected kept item.",
        "mark_rsvp_success" => "Reservation has been marked as 'No show'.",
        "make_reservation_success" => "Reservation has been made to ':table_no.'",
        "check_in_table_success" => "We've checked in the customers to selected table",
        "delay_rsvp_success" => "Selected reservation has been delayed to :date",
        "cancelled_rsvp_success" => "Selected reservation has been cancelled.",
        "admin_permission_granted" => "This admin now has access to ':permission'.",
        "admin_permission_restricted" => "Access to ':permission' has been restricted for this admin.",
        "admin_deleted" => "This admin has been deleted.",
        "add_success" => "Successfully added.",
        "add_tier_success" => "New tier has been added successfully.",
        "edit_tier_success" => "Tier has been edited successfully.",
        "delete_tier_success" => "Selected tier has been deleted successfully.",
        "delete_tier_error" => "Error deleting redeemable item.",
        "consolidate_invoice_cancelled" => "Consolidated Invoice Cancelled",
        "cancel_invoice_warning" => "Invoice cannot be cancelled after 72 hours",
        "something_wrong" => "Something went wrong, please try again later",
        "delete_printer_success" => "Printer has been successfully deleted.",
        "delete_printer_error" => "Delete printer failed",
        "test_delete_printer_error" => "Print failed",
        "app_connection_error" => "Failed to open STOXPOS app",
        "undetectable_printer_summary" => "Unable to detect selected printer",
        "undetectable_printer_detail" => "Please contact admin to setup the printer.",
        "invalid_printer_ip_summary" => "Invalid printer IP address",
        "invalid_printer_ip_detail" => "Please contact admin to setup the printer IP address.",
        "invalid_printer_port_summary" => "Invalid printer port number",
        "invalid_printer_port_detail" => "Please contact admin to setup the printer port number.",
        "add_printer_success" => "Printer has been added.",
        "edit_printer_success" => "Printer details has been edited.",
        "all_table_unlocked" => "All tables have been unlocked.",
        "update_auto_lock_duration_success" => "Auto-unlock durations successfully updated.",
        "update_point_calculation" => "Point calculation updated.",
        "update_point_expiration_settings" => "Point expiration settings updated.",
        "edit_merchant_details_success" => "Merchant details has been edited successfully.",
        "edit_merchant_address_success" => "Merchant address has been edited successfully.",
        "edit_cut_off_time_success" => "Cut-off time has been edited successfully.",
        "edit_taxes_success" => "Taxes edited successfully.",
        "copy_tin_number_success" => "TIN number copied successfully!",
        "add_promotion_summary" => "Promotion has been added.",
        "add_promotion_detail" => "Promotion will be visible to waiter and customer during the active period.",
        "edit_promo_success" => "Selected promotion has been edited successfully.",
        "delete_promo_success" => "Selected promotion has been deleted successfully.",
    ],

    // Action buttons' label
    "action" => [
        "add" => "Add",
        "apply" => "Apply",
        "cancel" => "Cancel",
        "change_image" => "Change image",
        "clear" => "Clear",
        "clear_all" => "Clear All",
        "confirm" => "Confirm",
        "export" => "Export",
        "leave" => "Leave",
        "login" => "Log in",
        "logout" => "Log Out",
        "next" => "Next",
        "previous" => "Previous",
        "save" => "Save",
        "stay" => "Stay",
        "take_me_back" => "Take me back",
        "refresh" => "Refresh",
        "done" => "Done",
        "go_to_homepage" => "Go to homepage",
        "check_in_customer" => "Check-in customer",
        "open_shift" => "{0} Open shift|{1} Open Shift",
        "go_to_add" => "Go to add",
        "serve" => "Serve",
        "free_up_table" => "Free Up Table",
        "make_payment" => "Make Payment",
        "mark_no_show" => "Mark as no show",
        "delay_reservation" => "Delay reservation",
        "cancel_reservation" => "Cancel reservation",
        "delete" => "Delete",
        "create_new" => "Create New",
        "more_action" => "More Action",
        "add_to_room" => "Add to room",
        "redeem_now" => "Redeem Now",
        "yes_sure" => "Yes, I'm sure",
        "remove" => "Remove",
        "no" => "No",
        "reassign" => "Reassign",
        "cancel_exit" => "Cancel & Exit",
        "reset" => "Reset",
        "unlock" => "Unlock",
        "proceed" => "Proceed",
        "print" => "Print",
        "apply_discount" => "Apply discount",
        "open_new_shift" => "Open new shift",
        "close_shift" => "Close shift",
        "open" => "Open",
        "close" => "Close",
        "pay_in" => "Pay in",
        "pay_out" => "Pay out",
        "save_changes" => "Save Changes",
        "discard" => "Discard",
        "new_product" => "New Product",
        "new_category" => "New Category",
        "new_waiter" => "New Waiter",
        "new_customer" => "New Customer",
        "maybe_later" => "Maybe Later",
        "return_item" => "Return Item",
        "return" => "Return",
        "adjust" => "Adjust",
        "subtract" => "Subtract",
        "replenish" => "Replenish",
        "view_stock_history" => "View Stock History",
        "new_group" => "New Group",
        "add_now" => "Add Now",
        "go_deactivate" => "Go deactivate",
        "new_item" => "New Item",
        "reactivate" => "Reactivate",
        "view_reservation_history" => "View Reservation History",
        "add_reservaion" => "Add Reservation",
        "edit" => "Edit",
        "check_in" => "Check in",
        "add_sub_admin" => "Add sub-admin",
        "go_back" => "Go back",
        "new_tier" => "New Tier",
        "another_reward" => "Another Reward",
        "consolidate" => "Consolidate",
        "submit" => "Submit",
        "ok" => "OK",
        "add_printer" => "Add Printer",
        "test_printer" => "Test Print",
        "unlock_all_table" => "Unlock All Table",
        "upload_photo" => "Upload Photo",
        "back" => "Back",
        "new_promotion" => "New Promotion",
    ],

    // Empty states' messages
    "empty" => [
        "no_data" => "No data can be shown yet...",
        "no_notification" => "No notification yet...",
        "no_result_found" => "We couldn't find any result...",
        "no_activity" => "No activity can be shown yet...",
        "no_reservation" => "You haven't added any reservations yet...",
        "no_shift_opened" => "No shift has been opened yet",
        "need_open_shift" => "You’ll need to open a shift before you can place order.",
        "add_table_room" => "You’ve to add a table/room before you can manage your order",
        "no_pending_item" => "No pending item to be served",
        "no_item_added" => "No item has been added to order",
        "no_pending_remove" => "No pending item to be removed",
        "no_bills" => "No bills have been paid yet...",
        "no_product_in_category" => "No product in this category yet...",
        "no_table_in_zone" => "No table in this zone...",
        "no_shift_record" => "You haven’t added any shift record yet...",
        "no_data_available" => "No data available",
        "no_table_added" => "You haven't added any table/room yet...",
        "no_product_added" => "You haven’t added any product yet...",
        "no_product_reassign" => "There are no products to reassign...",
        "no_employee_incentive_comm" => "This employee does not entitled to any incentive commission yet.",
        "no_waiter" => "You haven’t added any waiter yet...",
        "all_in_stock" => "Seems like everything is in stock!",
        "no_group" => "You haven't added any group yet...",
        "no_result" => "No result",
        "no_reservation_history" => "No reservation history for this guest",
        "no_redeemable_item" => "You haven't added any redeemable item yet...",
        "no_tier" => "You haven't added any tier yet...",
        "no_printer" => "You haven’t added any printer yet...",
        "no_promo" => "You haven’t added any promotion yet...",
    ],

    // Error pages' error messages
    "error" => [
        "error_404" => "Page does not exist",
        "error_404_message" => "We can’t seem to find the page you’re looking for. Meanwhile, please try again or go back to the previous page.",
        "error_408" => "Connection lost.",
        "error_408_message" => "It seems like your internet is on a vacation⛱️. Please try to reconnect your Wi-Fi or refresh the page to see if it helps.",
        "error_400" => "That’s not quite right",
        "error_400_message" => "It seems like your request got tangled up on its way here. Double-check the URL or try again.",
        "error_401" => "Access denied",
        "error_401_message" => "Looks like you’re trying to sneak in without a pass. Log in or provide the right credentials, and we’ll gladly open the gates for you.",
        "error_403" => "No peeking allowed",
        "error_403_message" => "Seems like this area is off-limits. Check with your administrator if you think you should have access.",
        "error_429" => "Slow down there!",
        "error_429_message" => "You’ve been sending requests faster than we can keep up! We’ve temporarily paused to catch our breath. Please give it a moment, and try again soon.",
        "error_503" => "Caught us tidying up",
        "error_503_message" => "Our developers are working hard to improve your experience with us. Sit back and have a sip of coffee ☕️, your journey will resume shortly.",
        "error_500" => "Something went wrong",
        "error_500_message" => "Our server ran into a hiccup while trying to process your request. We’re on it! In the meantime, feel free to refresh or come back later.",
        "error_502" => "Trouble on the bridge",
        "error_502_message" => "We’re having a bit of trouble communicating with another server. It’s not you—it’s us. Hang tight, and we’ll get it fixed ASAP.",
    ],

    // ========================================================================
    // ========================================================================

    
    // ========================================================================
    //                      PAGE-SPECIFIC TERMS
    // ========================================================================

    "login" => [
        "description1" => "Experience the ease of a hassle-free inventory management system today.",
        "description2" => "Your Inventory, Perfectly Managed",
        "id_placeholder" => "Enter your ID here",
        "password_placeholder" => "Enter your password here",
    ],
    "profile" => [
        "account_detail" => "Account Detail",
        "edit_display_name" => "Edit display name",
    ],
    "navbar" => [
        // For the main head titles, refer to global shared terms
        "latest_notification" => "Latest Notification",
        "view_all_notification" => "View all notification",
        "change_language" => "Change Language",
        "logout_message_title" => "You're leaving...",
        "logout_message_desc" => "Are you sure you want to log out this account?",
        "all_notification" => "All Notification",
        "notification" => "notification",
    ],
    "notification" => [
        "waiter_attendance" => "Waiter Check in / out",
    ],
    "dashboard" => [
        "sales_today" => "Sales Today",
        "products_sold_today" => "Products Sold Today",
        "orders_today" => "Orders Today",
        "low_in" => "Low in",
        "active_table_room" => "Active Table / Room",
        "on_duty_today" => "On Duty Today",
        "product_low_stock" => "Product Low at Stock",
        "today_reservation" => "Today's Reservation",
        "at_today" => "at Today",
    ],
    "order" => [
        "view_order_history" => "View Order History",
        "order_history" => "Order History",
        "enter_full_screen" => "Enter Full Screen",
        "empty" => "Empty",
        "in_use" => "In use",
        "pending_clearance" => "Pending Clearance",
        "seats" => "seats",
        "reserved" => "RESERVED",
        "search_by_order" => "Search by order number",
        "table_room" => "Table / Room",
        "order_completed_by" => "Order Completed By",
        "order_status" => "Order Status",
        "order_completed" => "Order Completed",
        "order_cancelled" => "Order Cancelled",
        "order_merged" => "Order Merged",
        "invoice" => "Invoice",
        "view" => "View",
        "all_orders" => "All Orders",
        "order_detail" => "Order Detail",
        "pending_serve" => "Pending Serve",
        "payment_history" => "Payment History",
        "total_exclude_tax" => "Total (excl. tax)",
        "ordered_by" => "Ordered by",
        "serve_later" => "Serve Later",
        "serve_now" => "Serve Now",
        "serve_now_qty" => "Serve Now Quantity",
        "no_order_yet" => "No order yet",
        "delete_order" => "Delete order",
        "delete_order_message_desc" => "Are you sure you want to delete the selected order? This action cannot be undone.",
        "delete_qty" => "Delete Quantity",
        "cancel_order_message_desc" => "Are you sure you want to cancel this order? All items will return to inventory and won't affect sales.",
        "cancel_order_redeemed_message_desc" => "Are you sure you want to cancel this order? Redeemed products/rewards won't be returned. Normal items will return to inventory and won't affect sales.",
        "view_price_breakdown" => "View Price Breakdown",
        "select_redeem_qty" => "Select Redeem Quantity",
        "redeem_reward" => "Redeem Reward",
        "existing_applied_reward" => "Existing Applied Reward Found",
        "redeem_reward_message1" => "Are you sure you want to redeem the selected reward",
        "redeem_reward_message2" => "for this customer",
        "replace_redeem_reward_message1" => "Are you sure you want to redeem and replace the currently applied reward with the selected reward",
        "replaced_amount_discount_message" => "The currently applied reward has been replaced with 'RM :discount_rate Discount' for this order.",
        "redeem_amount_discount_message" => "'RM :discount_rate Discount' has been applied to this order.",
        "replaced_percent_discount_message" => "The currently applied reward has been replaced with ':discount_rate% Discount' for this order.",
        "redeem_percent_discount_message" => "':discount_rate% Discount' has been applied to this order.",
        "redeem_free_item_message" => "':product_name' has been added to this customer's order.",
        "order_for" => "Order for :table_no",
        "stock_detail" => "Stock detail",
        "available_for_sale" => "Available for sale",
        "available_stock" => "Available stock",
        "check_in_guest" => "You've successfully check in customer to ':table_no'.",
        "not_allowed_keep_title1" => "Item(s) not allowed to keep.",
        "not_allowed_keep_desc" => "<strong>:count items</strong> will be removed from keep item listing.",
        "not_allowed_keep_title2" => "Item not allowed to keep",
        "max_keep_qty" => "You’ve reached the max keep quantity",
        "table_keep_history" => "Table's Keep History",
        "kept_for" => "Kept for",
        "kept_on" => "Kept on",
        "merge_with" => "Merge with",
        "reminder" => "Reminder",
        "merge_reminder" => "When you merge the tables, all the bills from the separate tables will be combined into one single bill.",
        "selected_tables" => "Selected Table(s)",
        "select_customer" => "Select a customer",
        "existing_customer" => "The table you’re merging with already has an existing customer",
        "choose_customer_stay" => "Please choose who will stay checked in after the table are merged.",
        "remove_reward" => "Remove Reward",
        "both_applied_rewards_removed" => "The applied reward for both orders will be removed, but you can always reapply it during payment.",
        "splt_applied_rewards_removed" => "Please note that splitting the bill will void any applied rewards, as rewards cannot be used with split bills.",
        "merged" => "Merged",
        "main_table" => "Main table",
        "table_merged_with" => "Table merged with :table_no",
        "reassign_order" => "{0} Reassign order|{1} Reassign Order",
        "reassign_message" => "Would you like to reassign the order from the main table to the unmerged table?",
        "current" => "Current",
        "move_to" => "Move to",
        "transfer_option_message" => "How would you like to transfer this table's order?",
        "transfer_all" => "Transfer all order",
        "transfer_items" => "Transfer item only",
        "transfer_to" => "Transfer to",
        "transfer_items_title" => "Transfer item to new table",
        "unlock_table" => "Unlock table?",
        "unlock_table_message" => "You still have items in your cart! Would you like to unlock table or stay?",
        "total_due" => "Total Due",
        "bill" => "Bill",
        "pay_this_bill" => "Pay this Bill",
        "pay_bill" => "Pay Bill",
        "balance_due" => "Balance Due",
        "exact" => "Exact",
        "merge_bill" => "{0} Merge bill|{1} Merge Bill",
        "split_bill" => "{0} Split bill|{1} Split Bill",
        "change" => "Change",
        "payment_success" => "Payment Successful",
        "no_receipt" => "No receipt",
        "paid_by" => "Paid by :method",
        "credit_debit_card" => "Credit/Debit Card",
        "discount_count" => ":count discount(s)",
        "add_bill" => "Add another bill",
        "applied" => "Applied",
        "all_methods" => "All methods",
        "only_methods" => ":method only",
    ],
    "shift" => [
        "shift" => "Shift",
        "no_shift" => "No data yet",
        "no_shift_message" => "Your shift record will show up here.",
        "no_shift_opened_message" => "Effortlessly keep an eye on the register shift at the store!",
        "starting_cash_amount" => "Starting cash amount",
        "enter_initial_cash" => "Please enter the initial cash amount in the register to begin your shift",
        "closing_cash_amount" => "Closing cash amount",
        "enter_current_cash" => "Please enter the current cash amount in your cash drawer.",
        "pay_in_out" => "Pay in/out",
        "pay_in_out_history" => "Pay in/out History",
        "open_date" => "Opening date",
        "opened_by" => "Opened by",
        "start_cash" => "Starting cash",
        "paid_in" => "Paid in",
        "paid_out" => "Paid out",
        "close_cash" => "Closing cash",
        "cash_diff" => "Cash difference",
        "close_date" => "Closing date",
        "closed_by" => "Closed by",
        "by" => "by",
        "pay_amount" => "The amount for pay in/out.",
        "pay_reason" => "Enter the reason you pay in/out.",
        "shift_pay_history" => "Your pay in/out history for this shift.",
        "closed" => "Closed",
        "shift_no" => "Shift No.",
        "open_time" => "Opening Time",
        "close_time" => "Closing Time",
        "net_sales" => "Net Sales",
        "shift_listing" => "Shift Listing",
        "shift_overview" => "Shift Overview",
        "print_report" => "Print Report",
        "cash_sales" => "Cash Sales",
        "card_sales" => "Card Sales",
        "e_wallet_sales" => "E-Wallet Sales",
        "gross_sales" => "Gross Sales",
        "discounts" => "Discounts",
        "net_sales_excl_tax" => "Net Sales (excl. taxes)",
        "shift_cash_drawer_detail" => "Shift &amp cash drawer detail",
        "cash_refunds" => "Cash refunds",
        "expected_cash" => "Expected cash",
    ],
    "menu" => [
        "availability" => "Availability",
        "total_product" => "Total Product",
        "top_selling_product" => "Top Selling Product",
        "price_range" => "Price Range",
        "show_redeemable_product" => "Show redeemable products only",
        "add_new_product" => "Add New Product",
        "manage_category" => "Manage Category",
        "delete_product" => "Delete this product?",
        "delete_product_message" => "Are you sure you want to delete the selected product? This action cannot be undone.",
        "edit_product" => "Edit Product Detail",
        "product_set_info" => "This product comes in a set (Bucket Product)",
        "product_detail" => "Product Detail",
        "sales_history" => "{0} Sales History|{1} Sales Histories",
        "redemption_history" => "{0} Redemption History|{1} Redemption Histories",
        "keep_in_qty_cm" => "Keep in quantity or cm",
        "keep_in_qty" => "Keep in quantity",
        "keep_in_cm" => "Keep in cm",
        "reassign_category" => "Reassign Category",
        "reassign_cat_message_title" => ":count items need to be re-assigned to another category",
        "reassign_cat_message" => "To proceed with removing this category, please assign a new category to each product.",
        "save_delete_category" => "Save & Delete Category",
        "delete_category" => "Delete this category?",
        "delete_category_message" => "Are you sure you want to delete this category? This action cannot be undone.",
    ],
    "report" => [
        "sales_summary" => "Sales Summary",
        "product_sales" => "Product Sales",
        "category_sales" => "Category Sales",
        "employee_earnings" => "Employee Earnings",
        "member_purchase" => "Member Purchase",
        "gross" => "Gross",
        "tax" => "Tax",
        "disc" => "Disc.",
        "net" => "Net",
        "no_of_sales" => "No.of Sales",
        "no_of_refund" => "No.of Refund",
        "employee" => "Employee",
        "total_purchase" => "Total Purchase",
        "no_of_purchase" => "No.of Purchase",
        "avg_spent" => "Avg. Spent",
        "stock_qty" => "Stock Qty",
        "kept_qty" => "Kept Qty",
        "sales_summary_report" => "Sales Summary Report",
        "payment_method_report" => "Payment Method Report",
        "product_sales_report" => "Product Sales Report",
        "category_sales_report" => "Category Sales Report",
        "employee_earn_report" => "Employee Earning Report",
        "member_purchase_report" => "Member Purchase Report",
        "current_stock_report" => "Current Stock Report",
    ],
    "inventory" => [
        "item_name" => "Item Name",
        "expire_in" => "Expire In",
        "total_group" => "Total Group",
        "total_item" => "Total Item",
        "item_out_of_stock" => "Out of Stock Item",
        "add_new_stock" => "Add New Stock",
        "inventory_summary" => "Inventory Summary",
        "delete_group" => "Delete this group?",
        "delete_group_message" => "All the item inside this group will be deleted altogether. Are you sure you want to delete this group?",
        "delete_group_warning" => "Some items in this group are in use!",
        "delete_group_warning_message" => "You cannot delete this group until you deactivate the products that include the items in this group. Would you like to proceed to deactivate those products?",
        "code" => "Code",
        "not_allowed" => "Not allowed",
        "add_new_group" => "Add New Group",
        "group_created" => "Group created!",
        "group_created_message" => "Do you want to add the item in the group to your menu?",
        "edit_group" => "Edit Group",
        "delete_item_reason" => "Reason for inventory item deletion",
        "stock_flow_detail" => "Stock Flow Detail",
        "reallocated" => "Reallocated",
        "kept_balance" => "Kept Balance",
        "remaining_stock" => "Remaining stock",
        "delete_item" => "Delete this item?",
        "delete_item_message" => "The products associated with this item cannot be reactivated, and this item cannot be recovered. This action cannot be undone. Are you sure you want to delete this item?",
        "delete_item_warning" => "This item is in use!",
        "delete_item_warning_message" => "You cannot delete this item until you deactivate the products that include it. Would you like to proceed to deactivate those products?",
        "stock_history" => "Stock History",
        "previous_stock" => "Previous Stock",
        "active_kept_item" => "Active Kept Item",
        "view_keep_history" => "View Keep History",
        "kept_date" => "Kept Date",
        "kept_in" => "Kept in",
        "keep_history" => "Keep History",
        "served_date" => "Served Date",
        "expired_date" => "Expired Date",
    ],
    "waiter" => [
        "add_waiter" => "Add New Waiter",
        "sales_performance" => "Sales Performance",
        "commission_earned" => "Commission Earned",
        "commission_amount" => "Commission Amount (RM)",
        "full_time" => "Full-time",
        "part_time" => "Part-time",
        "personal_detail" => "Personal Detail",
        "work_detail" => "Work Detail",
        "captain" => "Captain",
        "waiter_detail" => "Waiter Detail",
        "attendance" => "Attendance",
        "working_duration_header" => "Working Duration",
        "break_duration_header" => "Break Duration",
        "total_sales_this_month" => "Total sales this month",
        "commission_this_month" => "Commission in this month",
        "entitled_incentive" => "Entitled Incentive",
        "total_monthly_sales" => "of total monthly sales",
        "edit_waiter_detail" => "Edit Waiter Detail",
        "delete_waiter" => "Delete this waiter?",
        "delete_waiter_message" => "Are you sure you want to delete the selected waiter? This action cannot be undone.",
        "daily_sales_report" => "Daily Sales Report",
        "unknown_waiter" => "Unknown Waiter",
        "view_detail" => "View Detail",
        "monthly_commission_report" => "Monthly Commission Report",
        "monthly_sale" => "Monthly Sale",
        "monthly_incentive_report" => "Monthly Incentive Report",
        "incentive_reward_amount" => ":amount of total sales",
        "incentive_reward_percent" => ":percent of total sales",
        "est_hourly_rate" => "{0} Est. Rate|{1} Est. Rate (RM/hour)",
        "attendance_report" => "Attendance Report",
        "attendance_detail" => "Attendance Detail",
        "work_duration" => "Work",
        "break_duration" => "Break",
        "clock_out" => "Clock out",
        "break_end" => "Break end",
        "break_start" => "Break start",
        "clock_in" => "Clock in",
    ],
    "customer" => [
        "visited_customer" => "Visited Customer",
        "processing" => "Processing...",
        "import_keep_item" => "Import Keep Items",
        "with_keep_item_only" => "With keep item only",
        "no_tier" => "No Tier",
        "delete_customer" => "Passcode Required",
        "delete_customer_message" => "To delete this customer, you have to enter the passcode provided from the master admin.",
        "edit_customer" => "Edit Customer",
        "joined_date" => "Joined date",
        "customer_list" => "Customer List",
        "delete_customer_warning" => "This customer still has open orders!",
        "delete_customer_warning_message" => "You cannot delete this customer until all current orders are completed and paid. Would you like to proceed to view customer order?",
        "select_quantity" => "Select Quantity",
        "redeem_product_info" => "You can redeem the product only when the customer has checked-in to one table/room.",
        "adjust_point" => "Adjust Point",
        "adjust_amount_message" => "Please enter the amount of point you want to add to or substract from this customer's account.",
        "adjust_reason" => "Reason of adjust",
        "redeem_reward_info" => "You can redeem the rewards only when the customer has checked-in to one table/room.",
    ],
    "table" => [
        "manage_zone" => "Manage Zone",
        "add_table_room" => "Add Table / Room",
        "available_seat_numbers" => "No. of seats available",
        "select_zone" => "Select zone",
        "number_only" => "number only",
        "delete_table" => "Delete this table?",
        "delete_table_message" => "Are you sure you want to delete the selected table? This action cannot be undone.",
        "edit_table" => "Edit Table",
        "new_zone" => "New Zone",
        "add_zone" => "Add Zone",
        "delete_zone" => "Delete this zone?",
        "delete_zone_message" => "Table and room in this zone will be deleted altogether. Are you sure you want to delete this zone?",
        "delete_table_rsvp_warning" => "Cancel reservations",
        "delete_table_rsvp_warning_message" => "There are active reservations made for this table. Are you sure you want to delete the selected table? This action cannot be undone.",
    ],
    "reservation" => [
        "upcoming_reservation" => "Upcoming Reservation",
        "reservation_detail" => "Reservation Detail",
        "make_reservation" => "Make Reservation",
        "check_in_time" => "Check in time",
        "delayed_to" => "Delayed to",
        "completed_time" => "Completed time",
        "cancelled_on" => "Cancelled on",
        "checked_in_by" => "Checked in by",
        "update_by" => "Update by",
        "cancelled_by" => "Cancelled by",
        "cancellation_reason" => "Cancellation Reason",
        "edit_reservation" => "Edit Reservation",
        "guest_history" => "Guest History",
        "reservation_made_count" => "<strong>:count</strong> reservation has been made by this guest",
        "reservation_cancelled_count" => "<strong>:count</strong> reservation has been cancelled by this guest",
        "reservation_no_show_count" => "<strong>:count</strong> times the guest did not show up",
        "delete_reservation" => "Delete this reservation",
        "delete_reservation_message" => "Are you sure you want to delete the selected reservation? This action cannot be undone.",
        "conflicted_reservation_message" => "The selected tables are already occupied at the selected date and time.",
        "reservation_history" => "Reservation History",
        "unavailable_table_message" => "Some selected table/room are currently unavailable, please replace with another one.",
        "table_in_use_warning" => "Table in use",
        "table_in_use_message" => "The selected tables for the reservation are currently in use",
        "change_plan" => "Change of plan",
        "feeling_unwell" => "Feeling unwell",
        "bad_weather" => "Bad weather",
        "work_conflicts" => "Work conflicts",
        "family_emergency" => "Family emergency",
        "forgotten_reservation" => "Forgotten reservation",
        "other_reason" => "Others (specify under Remarks)",
    ],
    "transaction" => [
        "product_sold" => "Product Sold",
        "document_type_to_consolidate" => "Document type you’ll consolidate.",
        "last_month_transaction_unvalidated" => "Transaction which has not yet been validated from last month.",
        "submittable_transaction" => "Submittable Transaction",
        "transaction_unsubmitted" => "Transaction which has not yet been submitted to LHDN’s MyInvois for validation.",
        "submit_consolidated_invoice" => "Submit consolidated invoice",
        "sales_transaction_detail" => "Sales transaction detail",
        "points_given" => "Points Given",
        "count_kept_item_found" => ":count kept item found",
        "void_transaction" => "Void this transaction?",
        "void_transaction_warning" => "By voiding this transaction, the kept item(s) will be removed from the customer’s account. Are you sure you want to void this bill?",
        "void_transaction_effect" => "Voiding this transaction will remove it from active records and reverse any associated points or balances. This action cannot be undone.",
        "refund_limited" => "Refunds are limited to cost items only.",
        "list_excluded_items" => "Kept item, redeemed product, and entry reward item will not show in this list.",
        "select_refund_item" => "Select refund item",
        "count_item_kept" => ":count item kept",
        "fully_refunded" => "Fully Refunded",
        "refund_method" => "Refund Method",
        "enter_others_details" => "Enter others details",
        "refund_reason" => "Refund Reason",
        "enter_refund_reason" => "Enter refund reason",
        "total_refund" => "Total Refund",
        "refund_with_taxes" => "Refund with taxes",
        "heads_up" => "Heads-up!",
        "refund_regular_only" => "This refund applies to regular items only. Kept items will remain existing status and are not eligible for refund.",
        "refund_no" => "Refund No.",
        "refund_detail" => "Refund Detail",
        "refund_product" => "Refund Product",
        "refund_amount" => "Refund Amount",
    ],
    "einvoice" => [
        "transaction_list" => "Transaction List",
        "myinvois" => "MyInvois",
        "consolidated" => "Consolidated",
        "single_submission" => "Single Submission",
        "submission_date_time" => "Submmission Date & Time",
        "cancel_submission" => "Cancel Submission",
        "cancel_submission_warning" => "Are you sure you want to cancel this Consolidated Invoice submission?",
        "wrong_details" => "Wrong details",
        "general_public" => "General Public",
        "submission_detail" => "Submission detail",
        "transaction_total" => "Transaction Total",
        "amount_total" => "Amount Total",
        "myinvois_status" => "MyInvois Status",
        "submission_uuid" => "Submission UUID",
        "uuid" => "UUID",
        "issued_at" => "Issued at",
        "validated_at" => "Validated at",
    ],
    "loyalty" => [
        "member" => "Member",
        "icon" => "Icon",
        "tier_name" => "Tier Name",
        "achieve_spend_amount" => "Spend Amount to Achieve",
        "point_given_awaay" => "Points Given Away",
        "redemption_count" => "Counts of Redemption",
        "redeemable_item" => "Redeemable Item",
        "most_redeemed_product" => "Most Redeemed Product",
        "product_redemption_count" => ":count redemption",
        "recent_redemption" => "Recent Redemption",
        "maximise_customer_delight" => "Maximise Customer Delight",
        "maximise_customer_delight_message" => "Introducing more <span class='text-primary-900'>enchanting redeemable item</span> to your customer is one of the way to keep them engaged.",
        "redeemable_item_list" => "Redeemable Item List",
        "current_tier_list" => "Current Tier List",
        "add_new_tier" => "Add New Tier",
        "edit_tier" => "Edit Tier",
        "delete_tier" => "Delete this tier?",
        "delete_tier_message" => "All the member in this tier will not be entitled to any tier. Are you sure you want to delete this tier?",
        "remove_reward" => "Remove this reward?",
        "remove_reward_message" => "By removing this reward, new members entering this tier will no longer benefit from it. Are you sure you want to remove?",
        "edit_entry_reward_summary" => "Edit entry reward for this tier",
        "edit_entry_reward_detail" => "The changes will only be applied to new entry member only.",
        "tier_detail" => "Tier Detail",
        "spend" => "Spend",
        "entry_reward_for_this_tier" => "Entry Reward for this Tier",
        "member_name" => "Member name",
        "joined_on" => "Joined on",
        "total_spent" => "Total spent",
        "member_spending" => "Member Spending",
        "unknown_tier" => "Unknown tier",
        "member_list" => "Member List",
    ],
    "admin" => [
        "admin_user_list" => "Admin User List",
        "sub_admin_detail" => "Sub-admin detail",
        "allow_permission_access" => "Allow ':permission' access",
        "edit_sub_admin" => "Edit Sub-admin",
        "delete_admin" => "Delete this admin?",
        "delete_admin_message" => "Are you sure you want to delete the selected admin? This action cannot be undone.",
        "add_sub_admin_header" => "Add Sub-admin",
        "setup_sub_admin_detail" => "Set up sub-admin detail",
        "control_access" => "Control access",
    ],
    "analysis" => [
        "summary_report_header" => "Summary Reports",
        "total_product_sold" => "Total Product Sold",
        "total_order" => "Total Order",
        "order_summary_year" => "Year :year Order Summary",
        "order_count" => "Order Count",
        "order_summary" => "Order Summary",
        "sales_in_year" => "Year :year Sales in :category",
        "sales_in_category" => "Sales in Category",
        "current_period" => "Current Period",
        "last_period" => "Last Period",
    ],
    "logs" => [
        "cryptocurrency_market_price" => "Cryptocurrency",
    ],
    "config" => [
        "discount_settings" => "Discount Settings",
        "employee_commission" => "Employee Commission",
        "employee_incentive_programme" => "Employee Incentive Programme",
        "employee_shift_settings" => "Employee Shift Settings",
        "promotion" => "Promotion",
        "invoice_settings" => "Invoice Settings",
        "points_settings" => "Points Settings",
        "security_settings" => "Security Settings",
        "printing_settings" => "Printing Settings",
        "printer_name" => "Printer Name",
        "ip_address" => "IP",
        "port_number" => "Port",
        "open_cash_drawer" => "Open Cash Drawer",
        "printer_listing" => "Printer Listing",
        "edit_printer" => "Edit Printer",
        "enable_cash_drawer" => "Enable open cash drawer",
        "disable_cash_drawer" => "Disable open cash drawer",
        "delete_printer" => "Delete this printer?",
        "delete_printer_warning" => "The action that is linked to the printer will be unable to print anything. Are you sure you want to delete this printer?",
        "secs_of_inactivity" => "seconds of inactivity",
        "mins_of_inactivity" => "minutes of inactivity",
        "auto_lock_settings" => "Auto-Lock Settings",
        "point_calculation" => "Point Calculation",
        "point_expiration_behaviour" => "Point Expiration Behaviour",
        "points_expiration" => "Points Expiration",
        "expiration_notification" => "Expiration Notification",
        "days_after_added" => "days after added to account",
        "days_before_expiration" => "days before expiration",
        "tax_type" => "Tax type",
        "percentage" => "Percentage",
        "merchant_detail" => "Merchant Detail",
        "merchant_name" => "Merchant Name",
        "tin_no" => "TIN No.",
        "registration_no" => "Registration No.",
        "msic_code" => "MSIC Code",
        "sst_registration_no" => "SST Registration No.",
        "business_activity_desc" => "Business Activity Description",
        "irbm_client_id" => "IRBM Client ID",
        "irbm_client_key" => "IRBM Client Key",
        "classification_code" => "Classification Code",
        "search_tin" => "Search TIN",
        "merchant_address" => "Merchant Address",
        "address_line_1" => "Address Line 1",
        "address_line_2" => "Address Line 2",
        "postal_code" => "Postal Code",
        "area" => "Area",
        "state" => "State",
        "cutoff_time" => "Cut-off Time",
        "tax_settings" => "Tax Settings",
        "select_tin" => "Select TIN",
        "search_taxpayer_name" => "Search Taxpayer Name",
        "search_by_id_type" => "Search by ID Type<",
        "taxpayer_name" => "Taxpayer Name",
        "id_type" => "ID Type",
        "id_value" => "ID Value",
        "copy_tin_number" => "Copy the TIN number",
        "copied" => "Copied",
        "copy" => "Copy",
        "promotion_list" => "Promotion List",
        "add_new_promotion" => "Add New Promotion",
        "active_period" => "Active Period",
        "edit_promo" => "Edit Promotion",
        "delete_promo" => "Delete promotion?",
        "delete_promo_message" => "Are you sure you want to delete the selected promotion? This action cannot be undone.",
        "promo_desc_placeholder" => "promotion date and time, detail of the promotion and etc",
    ],

    // ========================================================================
    // ========================================================================
];
